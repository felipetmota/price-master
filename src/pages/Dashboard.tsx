...
export default function Dashboard() {
  ...
  const onImport = async (file: File) => {
    try {
      ...
      if (!data.prices.length) {
        toast.error("Nenhum registro encontrado na aba 'prices'.");
        return;
      }
      addPrices(data.prices);
      toast.success(`${data.prices.length} registro(s) importado(s).`);
    } catch (e) {
      console.error(e);
  ...
  const handleDelete = () => {
    ...
    setSelected(new Set());
    toast.success(`${confirmDelete.length} registro(s) removido(s).`);
    ...
    <AppLayout>
      <div className="space-y-6 animate-fade-in">
        <header className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">Preços contratuais</h1>
            <p className="text-sm text-muted-foreground mt-1">
              <span className="num">{stats.items}</span> itens · <span className="num">{stats.suppliers}</span> fornecedores · <span className="num">{stats.rows}</span> registros
            </p>
          </div>
        ...
        <a href="/templates/prices_template.xlsx" download className="inline-flex items-center gap-2 text-xs text-accent hover:underline">
          <FileSpreadsheet className="size-3.5" /> Baixar planilha modelo
        </a>
        ...
        {selected.size > 0 && (
          <div className="flex items-center justify-between rounded-lg border bg-accent-soft px-4 py-2.5 text-sm">
            <span><span className="font-medium num">{selected.size}</span> registro(s) selecionado(s)</span>
      ...
      <AlertDialog open={!!confirmDelete} onOpenChange={(v) => !v && setConfirmDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Excluir registro(s)?</AlertDialogTitle>
            <AlertDialogDescription>
              Esta ação remove {confirmDelete?.length ?? 0} registro(s) da memória. Para persistir, exporte a planilha após confirmar.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
